 ...
 * 
 * Version: 2.0.2 Copyright: (c) 2010-2011 Nicola Hibbert Licence: MIT
 * 
 ******************************************************************************/
(function(b) {
	var a = function(g, l) {
		var i = {
			...
			destroy : function() {
				...
				g.attr("style", "").removeClass(
						"accordion basic dark light stitch").removeData(
						"liteAccordion").unbind(".liteAccordion").find(
						"li > :first-child").unbind(".liteAccordion").filter(
			...
			setStyles : function() {
				g.width(h.containerWidth).height(h.containerHeight).addClass(
						"accordion").addClass(h.rounded && "rounded").addClass(
						h.theme);
				c.addClass("slide").children(":first-child").width(
						h.containerHeight).height(h.headerWidth).eq(
						h.firstSlide - 1).addClass("selected");
				j.each(function(m) {
					var p = b(this), o = m * h.headerWidth, n = j.first()
							.next(), q = parseInt(n.css("marginLeft"), 10)
							|| parseInt(n.css("marginRight"), 10) || 0;
					if (m >= h.firstSlide) {
						o += f
					}
			...
			linkable : function() {
				...
				var n = function(p) {
					...
					o = b.inArray(
							(window.location.hash.slice(1)).toLowerCase(), m);
					if (o > -1 && o < m.length) {
			...
			triggerSlide : function(p) {
				var o = b(this), m = j.index(o), n = o.next();
				d.currentSlide = m;
